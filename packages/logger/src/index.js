...
export function createLogger(options = {}) {
    const { component, logLevel = process.env.LOG_LEVEL || 'info', logToFile = process.env.LOG_TO_FILE === 'true', logFilePath = process.env.LOG_FILE_PATH || '/tmp/mmt.log', silent = false } = options;
    const transports = [];
    // Console transport
    if (!silent) {
        transports.push(new winston.transports.Console({
            level: logLevel,
            format: combine(colorize({ level: true }), timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), consoleFormat)
        }));
    }
...
export function formatError(error) {
    if (error instanceof Error) {
        return {
            ...
            stack: error.stack,
            name: error.name,
            ...error // Include any additional error properties
        };
    }
